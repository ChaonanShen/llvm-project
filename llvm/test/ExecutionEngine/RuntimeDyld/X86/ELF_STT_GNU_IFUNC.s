...
# "indirect_func" and "normal_func" that are not yet defined. They are called
# via the PLT to simulate how a compiler would emit a call to an external
# function.  Eventually, indirect_func will resolve to a STT_GNU_IFUNC and
# normal_func to a regular function. We include calls to both types of
# functions in this test test that both types of functions are executed
# correctly when their types not known initially.
# It also contains a call to a locally defined indirect function. As local
# functions are treated a bit differently by RuntimeDyld than external
# functions, we also test that.
...
# Test that the inderect functions have the same addresses in both calls.
