...
void RuntimeDyldELF::processNewSymbol(RTDyldSymbolTable::iterator SymbolIt) {
  auto& Symbol = SymbolIt->second;
  if (Symbol.getFlags().isIndirect()) {
...
void RuntimeDyldELF::createIFuncResolver(uint8_t *Addr) const {
  if (Arch == Triple::x86_64) {
    ...
    // clang-format on
    assert(sizeof(StubCode) <= 64 && "maximum size of the IFunc resolver is 64B");
...
void RuntimeDyldELF::createIFuncStub(unsigned IFuncStubSectionID,
  ...
  if (Arch == Triple::x86_64) {
    ...
    report_fatal_error(
        "IFunc stub is not supported for target architecture");
  }
}
